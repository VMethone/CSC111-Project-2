...
def load_dataset(filepath="Final.csv"):
    ...
    df = df.dropna(subset=["city1", "city2", "airport_1", "airport_2", "fare", "nsmiles"])
    df["period"] = list(zip(df["year"], df["quarter"]))
    return df

def compute_route_stats(df):
    stats = defaultdict(lambda: {'fares': [], 'passengers': [], 'distance': [], 'frequency': 0})
    for _, row in df.iterrows():
        ...
        stats[route]['distance'].append(row["nsmiles"])
...
def apply_business_rules(df):
    df['pred_passengers'] = np.where(df['pred_passengers'] < MIN_PASSENGERS, 0, np.round(df['pred_passengers']))
...
def export_predictions(G, period, df_template):
    records = []
    city_map_1 = df_template[['airport_1', 'city1']].drop_duplicates().set_index('airport_1')['city1'].to_dict()
    city_map_2 = df_template[['airport_2', 'city2']].drop_duplicates().set_index('airport_2')['city2'].to_dict()
    ...
    for u, v, data in G.edges(data=True):
        records.append({
            ...
            'airport_1': u,
            'airport_2': v,
            'city1': city_map_1.get(u, None),
            'city2': city_map_2.get(v, None),
            'fare': data['fare'],
    ...
    df_out = df_out.dropna(subset=["airport_1", "airport_2", "city1", "city2", "fare", "passengers"])
    # df_out = df_out[df_out["distance"] > 0]  # optional: filter invalid distances

    if df_out.empty:
        print(f"⚠️ WARNING: Exported DataFrame for {period} is empty after filtering!")
...
def main():
    ...
    for (year, quarter), group in df.groupby(['year', 'quarter']):
        ...
        for _, row in group.iterrows():
            G.add_edge(row['airport_1'], row['airport_2'], weight=row['fare'], passengers=row['passengers'])
    ...
    for i in range(1, len(sorted_periods)):
        ...
        features['exists'] = features['route'].apply(lambda r: int(r in curr_edges or tuple(reversed(r)) in curr_edges))
        features['fare'] = features['route'].apply(lambda r: curr_edges.get(r, {}).get('weight', 0) or curr_edges.get(tuple(reversed(r)), {}).get('weight', 0))
        features['passengers'] = features['route'].apply(lambda r: curr_edges.get(r, {}).get('passengers', 0) or curr_edges.get(tuple(reversed(r)), {}).get('passengers', 0))

        feature_rows.append(features)

    full_features = pd.concat(feature_rows)

    route_model = RandomForestClassifier(n_estimators=100, max_depth=8, class_weight='balanced', random_state=42)
    route_model.fit(full_features[['existed_last_q', 'degree_u', 'degree_v', 'hub_u', 'hub_v']], full_features['exists'])

    fare_pipeline = get_fare_pipeline()
    fare_pipeline.fit(full_features[full_features['exists'] == 1][['distance', 'hub_u', 'hub_v', 'pagerank_u', 'pagerank_v', 'avg_fare', 'fare_variation', 'frequency']],
                      full_features[full_features['exists'] == 1]['fare'])

    pax_pipeline = get_pax_pipeline()
    pax_pipeline.fit(full_features[full_features['exists'] == 1][['distance', 'hub_u', 'hub_v', 'passenger_range', 'frequency']],
                     full_features[full_features['exists'] == 1]['passengers'])
    ...
    for period in future_periods:
        ...
        pred_features['exists_pred'] = route_model.predict(pred_features[['existed_last_q', 'degree_u', 'degree_v', 'hub_u', 'hub_v']])
        active_routes = pred_features[pred_features['exists_pred'] == 1]

        fare_pred = fare_pipeline.predict(active_routes[['distance', 'hub_u', 'hub_v', 'pagerank_u', 'pagerank_v', 'avg_fare', 'fare_variation', 'frequency']])
        pax_pred = pax_pipeline.predict(active_routes[['distance', 'hub_u', 'hub_v', 'passenger_range', 'frequency']])

        pred_features = pred_features.merge(
            pd.DataFrame({'route': active_routes['route'], 'pred_fare': fare_pred, 'pred_passengers': pax_pred}),
            on='route', how='left'
        ).fillna(0)
        ...
        for _, row in final_pred.iterrows():
            if row['pred_passengers'] > 0:
                u, v = row['route']
                G_pred.add_edge(u, v, fare=round(row['pred_fare'], 2), passengers=int(row['pred_passengers']), distance=row.get('distance', 0))
